...
test('PRISMA navigation works', async ({ page }) => {
  ...
  const main = page.locator('main.prisma-main').filter({ visible: true });
  ...
  await expect(page.getByText('Evidência de aprendizagem').last()).toBeVisible();
  await page.locator('.back').first().click();
  await page.getByRole('button', { name: /Aluno/i }).click();
  await page.locator('[data-clean-tab="curriculo"]').click();
  await expect(page.locator('main.prisma-main').last()).toBeVisible();
